-- Empeche une sortie de caisse ou un virement de vider un compte qui n'a pas
-- assez d'argent. Le solde d'un compte est la somme des ledger_entries dont
-- le cash_account_id (normalise) correspond.
--
-- Constat : les ventes/paiements utilisent des libelles de moyen de paiement
-- ("Cash", "Virement", "Cheque") differents des libelles de compte utilises par
-- les mouvements manuels ("Caisse principale", "Banque") pour designer le meme
-- compte physique. On normalise donc avant de comparer, sinon une sortie
-- legitime serait refusee a tort (l'argent d'une vente "Cash" ne serait pas vu
-- comme appartenant a "Caisse principale").

CREATE OR REPLACE FUNCTION private.normalize_cash_account(p_account text)
RETURNS text
LANGUAGE sql
IMMUTABLE
SET search_path = public
AS $function$
  SELECT CASE btrim(COALESCE(p_account, ''))
    WHEN 'Cash' THEN 'Caisse principale'
    WHEN 'Virement' THEN 'Banque'
    WHEN 'Chèque' THEN 'Banque'
    WHEN 'Cheque' THEN 'Banque'
    ELSE btrim(COALESCE(p_account, ''))
  END;
$function$;

REVOKE ALL ON FUNCTION private.normalize_cash_account(text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION private.normalize_cash_account(text) TO authenticated, service_role;

CREATE OR REPLACE FUNCTION private.cash_account_balance(p_company_id uuid, p_account text)
RETURNS numeric
LANGUAGE sql
STABLE
SET search_path = public
AS $function$
  ...
  WHERE company_id = p_company_id
    AND status = 'posted'
    AND private.normalize_cash_account(cash_account_id) = private.normalize_cash_account(p_account);
$function$;

REVOKE ALL ON FUNCTION private.cash_account_balance(uuid, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION private.cash_account_balance(uuid, text) TO authenticated, service_role;

-- record_cash_movement : bloque une sortie si le compte source n'a pas assez.
...
BEGIN
  ...
  -- Idempotent replay: si cette cle existe deja, on ne revalide pas le solde
  -- (l'operation a deja ete faite), on renvoie juste l'ecriture existante plus bas.
  IF NOT EXISTS (
    ...
    IF v_balance < p_amount THEN
      RAISE EXCEPTION 'Solde insuffisant sur % : % disponible, % demande',
        p_cash_account_id, v_balance, p_amount
        USING ERRCODE = '23514';
    END IF;
  END IF;
...
-- record_cash_transfer : bloque si le compte source n'a pas assez.
...
BEGIN
  ...
  -- Idempotent replay: ne revalide pas le solde si cette operation a deja ete faite.
  IF NOT EXISTS (
