-- record_payroll_payment sortait de la caisse sans jamais verifier le solde
-- disponible -- seule RPC de sortie de caisse a echapper au garde-fou deja
-- en place sur record_cash_movement/record_cash_transfer depuis
-- 20260923090000_add_cash_balance_guard.sql. Avec l'arrivee du paiement en
-- lot des salaires (roadmap item 20), le risque de decouvrir un compte sans
-- avertissement devient bien plus concret (plusieurs sorties a la suite).
-- Meme patron exact que les autres RPC de caisse : solde verifie uniquement
-- sur le premier passage (idempotent_key deja connue = retry, on ne
-- reverifie pas), message d'erreur avec solde disponible/demande + conseil
-- de virement.
...
BEGIN
  ...
  -- Nouveau paiement (pas un retry) : verifier le solde du compte de
  -- paiement avant de sortir quoi que ce soit -- meme patron que
  ...
  IF v_balance < v_net THEN
    RAISE EXCEPTION 'Solde insuffisant sur %', private.normalize_cash_account(v_method)
      USING ERRCODE = '23514',
            DETAIL = format(
              'Disponible : %s FCFA. Montant du salaire : %s FCFA.',
              to_char(v_balance, 'FM999G999G999G999'),
              to_char(v_net, 'FM999G999G999G999')
            ),
            HINT = format(
              'Faites d''abord un virement vers %s depuis un autre compte (Caisse principale, Wave, Orange Money, Banque), puis réessayez ce paiement.',
              private.normalize_cash_account(v_method)
            );
  END IF;
